...
next 3 days of PM2.5 -> AQI, and shows a dashboard with alerts.
...
def fetch_live_data():
    """Recent hourly air-quality and weather data (past 15 days) from Open-Meteo."""
...
st.markdown(f"""
<div class="hero">
  <h1>{CITY_NAME} Air Quality Forecast</h1>
...
try:
    ...
    st.error(f"Model file '{MODEL_PATH}' was not found. Place it in the same folder as "
             "this app (it is produced by the training notebook), then refresh.")
    st.stop()
...
if feat.empty:
    st.error("Not enough recent data to build features.")
    st.stop()
...
with c1:
    ...
    <div class="card" style="background:{aqi_color(current_aqi)};">
        ...
        <div class="pm">PM2.5: {current_pm:.1f} ug/m3</div>
...
for i, col in enumerate(cols):
    ...
    with col:
        ...
        <div class="card" style="background:{aqi_color(aqi)};">
            ...
            <div class="pm">PM2.5: {pm:.1f} ug/m3</div>
        </div>""", unsafe_allow_html=True)
...
if worst > 200:
    st.error(f"Hazardous air quality expected (AQI {worst}). Avoid outdoor activity and wear a mask.")
elif worst > 150:
    st.warning(f"Unhealthy air quality expected (AQI {worst}). Limit time outdoors.")
elif worst > 100:
    st.info(f"AQI may reach {worst}. Sensitive groups should take precautions.")
else:
    st.success(f"Air quality is expected to stay acceptable (up to AQI {worst}).")
...
especially during sudden pollution spikes. AQI is computed from PM2.5 (US EPA scale).
</div>""", unsafe_allow_html=True)
